...
class Genetic:
    ...
    def next_gen(self):
        ...
        while len(self.gen) < self.num_pop:
            if np.random.rand()>0.5:
                ...
                dino1,dino2 = np.random.choice(self.gen,size=2,replace=False)
    ...
    def evaluate(self):
        ...
        history.append(np.array(self.best_fitness).mean)

def train(num_gen=10,num_dino=100,fps=30):
    global game_speed, x_pos_bg, y_pos_bg, obstacles, dinosaurs, points, history
    ...
    for num in range(num_gen):
        ...
        def score():
            ...
            text_2 = FONT.render(f'Highest Fitness:{genetic.best_score:.1f}',True,(0,0,0))
